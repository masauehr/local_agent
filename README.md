...
### 2026-08-13 - Ollamaサーバー再起動の必要性

**質問:** ローカルLLMを使ったClaude Codeを起動し直すには、Ollamaサーバーの再起動が必要か？

**回答:**
いいえ、不要です。`ollama serve` はバックグラウンドで動き続ける常駐サーバーで、Claude Codeはそこに毎回HTTP接続しているだけのクライアントです。

`start.sh` 自体も冒頭で `curl http://localhost:11434/api/tags` により起動確認し、既に起動していればスキップする作りになっています（未起動の場合のみ自動起動）。

Ollamaサーバーの再起動が必要になるのは、次のような場合だけです。
- サーバーがハング・クラッシュしている
- `brew upgrade ollama` 等でOllama本体を更新した直後（新しいバイナリを反映するため）
- `OLLAMA_*` 系の環境変数（例: コンテキスト長設定等）を変更し、それを反映させたい場合

モデルの切り替えやMCP設定変更（今回の `mcp-jma-only.json` 追加など）は、サーバー再起動なしで即時反映されます。
