...
| モデル | 取得方法 | サイズ |
|--------|---------|-------|
| `qwen3.6:35b-mlx` | Ollama MLX | 21GB |
| `gemma4:31b-mlx` | Ollama MLX（要0.20.2以降） | 20GB |
| `RakutenAI-2.0-8x7B Q8_0` | HuggingFace GGUF | 49GB |
| `RakutenAI-2.0-8x7B Q4_K_M` | HuggingFace GGUF | 28.5GB |
| `devstral-small-2:24b` | Ollama | 15GB |

### 総合評価表

| 評価項目 | qwen3.6 | gemma4 | RakutenAI | devstral |
|---------|---------|--------|-----------|---------|
| 日本語の自然さ | △ 英語混入 | △ 英語の思考過程が出る | ◎ 最良 | △ |
| 知識の正確さ | ✅ | ✅ | ✅ | ✅ |
| 論理的思考 | ✅ | ✅ | ❌ | △ |
| コード生成（チャット） | ○ | ○ | ❌ 拒否 | △ |
| テトリスコード品質 | ★★★★（壁蹴り・レベル・ゴースト実装） | ★☆（インデントエラーで実行不可） | — | ★★（複数行クリアバグあり） |
| Claude Code 操作感 | ◎ 本物に近い | △ | ❌ ツール非対応 | △ 遅い |
| ファイル書き出し | ✅ 問題なし | ❌ ハングアップ | ❌ ツール非対応 | ❌ 非常に遅い |
| Claude Code 実用性 | △ コード修正は不安定 | ❌ | ❌ | ❌ |

### 用途別最終推奨

| 用途 | 推奨モデル | 理由 |
|------|-----------|------|
| **Claude Code / AIエージェント** | `qwen3.6:35b-mlx` | 操作感が本物に最も近い・ファイル操作安定 |
| **日本語チャット（コード以外）** | `RakutenAI Q4_K_M` or `Q8_0` | 日本語の自然さが最良・知識も十分 |
| **日本語チャット（コード含む）** | `gemma4:31b-mlx` | 論理・コード生成ともに水準あり |

### 総括コメント

> ローカルLLMで Claude Code を実用レベルで使うにはまだ力不足。
